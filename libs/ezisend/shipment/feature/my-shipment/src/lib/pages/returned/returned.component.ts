...
export class ReturnedComponent implements OnInit {
    ...
    switch (event.actionType) {
      ...
      case 'order-details':
        this._commonService.googleEventPush({
          ...
          // parcel_width: "Parcel Width e.g. 19.00",
          // parcel_height: "Parcel Height e.g. 19.00",
          // parcel_length: "Parcel Length e.g. 10.50",
          // parcel_volumetric_weight: "Parcel Volumetric Weight e.g. 0.7581",
          // item_description: "Item Description e.g. Traditional medicine",
          // premium_amount: "Premium Amount e.g. 1",
        });
        window.open(urlDetails);
        return;
      default:
  ...
  onDownloadAsAndPrint(event: string, isMultiple = false) {
    let shipmentIds: number[] = [];
    // console.log(this.batchProcessingIds)
